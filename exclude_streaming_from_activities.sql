-- Exclude Streaming from Activities Page Functions
-- "Move screen sharing statistics from the 'Atividades' page to the 'Destaques' page"

-- 1. get_top_activities (Exclude Streaming)
...
AS $$
  ...
  WHERE a.guild_id = p_guild_id 
    ...
    AND u.is_bot = FALSE
    AND a.activity_type != 'STREAMING'   -- EXCLUDE STREAMING
    AND a.activity_name != 'Streaming'   -- EXCLUDE STREAMING NAME Just in case
  GROUP BY a.activity_name
  ORDER BY total_seconds DESC
  LIMIT p_limit;
$$;

-- 2. get_daily_activity_stats (Exclude Streaming)
...
AS $$
  ...
  WHERE a.guild_id = p_guild_id
    ...
    AND a.duration_seconds > 0
    AND u.is_bot = FALSE
    AND a.activity_type != 'STREAMING'
    AND a.activity_name != 'Streaming'
  GROUP BY 1
  ORDER BY 1;
$$;

-- 3. get_top_users_by_activity (Exclude Streaming unless explicitly requested?? No, default exclude)
...
AS $$
  ...
  WHERE a.guild_id = p_guild_id
    ...
    AND u.is_bot = FALSE
    -- Only exclude streaming if p_activity_name is NULL (general top users). 
    -- If p_activity_name is 'Streaming', user logically wants it (though UI won't request it).
    AND (p_activity_name IS NOT NULL OR (a.activity_type != 'STREAMING' AND a.activity_name != 'Streaming'))
  GROUP BY u.user_id, u.username, u.discriminator
  ORDER BY total_seconds DESC
  LIMIT p_limit;
$$;

-- 4. get_activity_type_distribution (Exclude Streaming)
