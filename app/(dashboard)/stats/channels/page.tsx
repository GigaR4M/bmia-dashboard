...
import { useTopChannels } from '@/hooks/useStats'
import { formatNumber, formatDateTime } from '@/lib/utils'
import { Hash } from 'lucide-react'

export default function ChannelsPage() {
    const { data: channels, loading } = useTopChannels(20)
                            ...
                            {loading ? (
                                ...
                                channels.map((channel, index) => (
                                    <tr key={channel.channel_id} className="hover:bg-slate-700/30 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-white font-semibold">{formatNumber(channel.message_count)}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-slate-400 text-sm">{formatDateTime(channel.last_message_at)}</span>
                                        </td>
                                    </tr>
                                ))
                            )}
